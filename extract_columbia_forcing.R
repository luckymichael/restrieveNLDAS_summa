#!/opt/R/bin/Rscript --vanilla --default-packages=utils
...
# calculate the 1D index of the cell
...
convertNLDAS2NC <- function(NLurl,NLfname,NCfname,t){
  ...
  download.file(NLurl,NLfname)
  # read the grib file, alternative using rNOMADS package: grib <- ReadGrib(file.name = NLfname, levels = 1,variables = c("TMP2m"))
...
# function to download the NLDAS file and store the necessrary data
...
# 
# NLDAS.index.2Dto1D <- function(x,y){
#   
# }
# 
# 
...
# y <- ncdim_def( name="Lat","", vals=as.double(-89:89))
# t <- ncdim_def(name="Time", units="days since 1900-01-01", vals=1:10, unlim=TRUE, calendar="standard", longname="Observation time")
