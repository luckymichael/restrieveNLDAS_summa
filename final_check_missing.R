...
foreach(this.day=all.date) %do% {
  ...
  #print(paste("Checked",format(this.day,"%Y%m%d.")))
}


# check each folder 
allmonth <- seq(start.date,end.date,by="month")
num.month <- length(allmonth)
allmonth <- c(allmonth, as.Date("2016-01-01"))
foreach(m = 1:num.month) %do% {
  mon <- allmonth[m]
  num.day <- as.integer(allmonth[m+1] - mon)
  files <- dir(paste0(format(mon,"%Y%m%d."),"/*.nc"))
  if (length(files)/24 == num.day) print(paste("Checked",format(this.day,"%Y%m%d."))) else{
    # check missing file
